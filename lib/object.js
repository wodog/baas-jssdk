...
module.exports = Baas => {
  ...
  Baas.Object.prototype.fetch = function() {
    ...
    return Baas.api.get(`/tables/${this.tableName}/${_id}`).then(data => {
  ...
  Baas.Object.prototype.save = function() {
    ...
    return Baas.api[method](`/tables/${this.tableName}` + this._id ? `/${this._id}` : '', this.$model);
  };
  Baas.Object.prototype.destroy = function() {
    const { _id } = this;
    if (!_id) throw new InvalidParamsError('_id 属性不能为空');
    return Baas.api.delete(`/tables/${this.tableName}/${_id}`);
  };
