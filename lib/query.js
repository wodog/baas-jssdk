...
module.exports = function(Baas) {
  ...
  Query.prototype.find = function(query) {
    return Baas.api.get('/tables/' + this.tableName, {
      params: { query: query || this.queryBuilder }
    }).then(function(res) {
      return transformResponse(this.tableName, res.data);
    });
  };
  Query.prototype.get = function(id) {
    if (!id) throw new InvalidParamsError('id 不能为空');
    return Baas.api.get('/tables/' + this.tableName + '/' + id).then(function(res) {
      return transformResponse(this.tableName, res.data);
    });
  };
